...
public class AuthHandler
{
    ...
    {
        manager.curChar = chr;
        PacketWriter outpacket = new PacketWriter(WorldServerOpCode.CMSG_PLAYER_LOGIN);
        outpacket.Write(chr.GUID);
        manager.Send(outpacket);

        outpacket = new PacketWriter(WorldServerOpCode.CMSG_UPDATE_ACCOUNT_DATA);
        outpacket.Write((UInt32)7);
        outpacket.Write((UInt32)5144);
        outpacket.Write(account_data);
        manager.Send(outpacket);

        manager.objectMgr.MapID = chr.MapID;
        manager.objectMgr.playerGuid = new WoWGuid(chr.GUID);
        manager.objectMgr.getPlayerObject().SetPlayer(chr);
        //manager.StartHeartbeat();

        //manager.movementMgr.Start();
        //manager.combatMgr.Start();
        manager.terrainMgr.ChangeMap(chr.MapID);


        //Exchange.OnNewLog("Logged into world as " + chr.Name);
    ...
    public static void HandleCharCreate(Character c, World manager)
    {
        PacketWriter outpacket = new PacketWriter(WorldServerOpCode.CMSG_CHAR_CREATE);

        outpacket.Write(c.Name);
        outpacket.Write(c.Race);
        outpacket.Write(c.Class);
        outpacket.Write(c.Gender);
        outpacket.Write(c.Skin);
        outpacket.Write(c.Face);
        outpacket.Write(c.HairStyle);
        outpacket.Write(c.HairColour);
        outpacket.Write(c.FacialHair);
        outpacket.Write(c.OutfitID);
